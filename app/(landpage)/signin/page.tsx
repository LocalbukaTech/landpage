...
const SignInContent = () => {
  ...
  const [authView, setAuthView] = useState<"signin" | "forgot" | "reset">(
    "signin",
  );
        ...
        onSuccess: async (response) => {
          ...
          try {
            ...
            if (Array.isArray(prefs)) {
              ...
              router.push(
                `/signup/preferences?flow=login&redirect=${encodeURIComponent(redirect)}`,
              );
            }
          } catch (err) {
            console.log(
              "No preferences found or error fetching, redirecting to onboarding:",
              err,
            );
            router.push(
              `/signup/preferences?flow=login&redirect=${encodeURIComponent(redirect)}`,
