...
import {useDeleteRestaurant, useMyRestaurants, useUpdateRestaurant} from "@/lib/api/services/restaurants.hooks";
...
const INPUT_CLASS = "w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none focus:border-[#fbbe15]/50 focus:ring-1 focus:ring-[#fbbe15]/20 transition-all placeholder:text-zinc-600";

// ── Collapsible section wrapper (extracted to file-level to avoid react-hooks/static-components) ──
function AccordionSection({ icon: Icon, title, subtitle, isOpen, onToggle, children }: {
  icon: any; title: string; subtitle: string; isOpen: boolean; onToggle: () => void; children: React.ReactNode;
}) {
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
      <button onClick={onToggle} className="w-full px-5 py-4 flex items-center gap-3 hover:bg-white/2 transition-colors">
        ...
        <div className="flex-1 text-left">
          <span className="text-sm font-bold text-white block leading-tight">{title}</span>
...
function EditModal({ restaurant, onClose, onSaved }: EditModalProps) {
  ...
  const [isClaimedByOwner, setIsClaimedByOwner] = useState(restaurant.isClaimedByOwner ?? true);
  ...
  // ── Photos ──
  const [existingPhotos, setExistingPhotos] = useState<string[]>(restaurant.photos || []);
  ...
  const [openingHours, setOpeningHours] = useState<Record<string, string>>(() => {
    ...
    DAYS.forEach((day) => { hours[day] = existing[day] || ""; });
    return hours;
  });
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    basic: true, location: false, contact: false, cuisine: false, price: false, photos: false, hours: false, ownership: false,
  });
  ...
  const handleSave = async () => {
    ...
    const cuisineStr = [
      ...selectedCuisines,
      ...customCuisine.split(",").map((c) => c.trim()).filter(Boolean),
    ].join(",");
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-[#1e1e1e] border border-white/10 rounded-2xl shadow-2xl scrollbar-hide">
        ...
        <div className="p-5 space-y-3">
          {/* ── Basic Info ── */}
          <AccordionSection icon={Store} title="Basic Information" subtitle="Name & description" isOpen={openSections.basic} onToggle={() => toggle("basic")}>
            <div>
              <label className="block text-sm font-semibold text-zinc-300 mb-2">Restaurant Name</label>
          ...
          {/* ── Location ── */}
          <AccordionSection icon={MapPin} title="Location" subtitle="Address & coordinates" isOpen={openSections.location} onToggle={() => toggle("location")}>
            <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input type="text" value={state} onChange={(e) => setState(e.target.value)} className={INPUT_CLASS} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-semibold text-zinc-300 mb-2">Latitude <span className="text-zinc-600">(optional)</span></label>
                <input type="text" value={lat} onChange={(e) => setLat(e.target.value)} placeholder="6.5244" className={INPUT_CLASS} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-zinc-300 mb-2">Longitude <span className="text-zinc-600">(optional)</span></label>
                <input type="text" value={lng} onChange={(e) => setLng(e.target.value)} placeholder="3.3792" className={INPUT_CLASS} />
              </div>
            </div>
          </AccordionSection>

          {/* ── Contact ── */}
          <AccordionSection icon={Phone} title="Contact & Links" subtitle="Phone & website" isOpen={openSections.contact} onToggle={() => toggle("contact")}>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+234 800 123 4567" className={`${INPUT_CLASS} pl-11`} />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="url" value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://yoursite.com" className={`${INPUT_CLASS} pl-11`} />
                </div>
              </div>
            </div>
          </AccordionSection>
          ...
          {/* ── Cuisine ── */}
          <AccordionSection icon={ChefHat} title="Cuisine Type" subtitle="Select or type cuisines" isOpen={openSections.cuisine} onToggle={() => toggle("cuisine")}>
            <div className="flex flex-wrap gap-2">
              {POPULAR_CUISINES.map((c) => (
                ...
                >
                  {selectedCuisines.includes(c) && <Check size={12} className="inline mr-1 -mt-0.5" />}
                  {c}
                </button>
              ))}
            </div>
            <div>
              <label className="block text-xs text-zinc-500 mb-2">Or type custom cuisines (comma-separated)</label>
              <input type="text" value={customCuisine} onChange={(e) => setCustomCuisine(e.target.value)} placeholder="e.g. Suya, Pepper Soup" className={INPUT_CLASS} />
            </div>
          </AccordionSection>

          {/* ── Price Level ── */}
          <AccordionSection icon={DollarSign} title="Price Level" subtitle="How pricy is the experience?" isOpen={openSections.price} onToggle={() => toggle("price")}>
          ...
          {/* ── Photos ── */}
          <AccordionSection icon={ImagePlus} title="Photos" subtitle={`${totalPhotos}/10 uploaded`} isOpen={openSections.photos} onToggle={() => toggle("photos")}>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              ...
              {newPhotoUrls.map((url, i) => (
                <div key={`new-${i}`} className="relative aspect-square rounded-xl overflow-hidden group ring-2 ring-[#fbbe15]/30">
                  <Image src={url} alt={`New photo ${i + 1}`} fill className="object-cover" sizes="150px" />
                  <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 bg-[#fbbe15] rounded text-[9px] font-bold text-[#1a1a1a]">NEW</div>
                  <button
            ...
            {removedPhotos.length > 0 && (
              <p className="text-xs text-zinc-500">
                {removedPhotos.length} photo{removedPhotos.length !== 1 ? "s" : ""} will be removed on save
              </p>
            )}
          </AccordionSection>

          {/* ── Opening Hours ── */}
          <AccordionSection icon={Clock} title="Opening Hours" subtitle="Leave blank for closed days" isOpen={openSections.hours} onToggle={() => toggle("hours")}>
            <div className="space-y-2.5">
              {DAYS.map((day) => (
                <div key={day} className="flex items-center gap-3">
                  <span className="w-20 text-sm font-semibold text-zinc-300 capitalize">{day}</span>
                  <input
                    type="text"
          ...
          {/* ── Ownership ── */}
          <AccordionSection icon={Info} title="Ownership" subtitle="Claim this listing" isOpen={openSections.ownership} onToggle={() => toggle("ownership")}>
            <div className="flex items-center justify-between">
              <span className="text-sm text-zinc-300">I own this restaurant</span>
              <button
              ...
              >
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${isClaimedByOwner ? "translate-x-7" : "translate-x-1"}`} />
              </button>
            </div>
          </AccordionSection>
        </div>
        ...
        <div className="sticky bottom-0 bg-[#1e1e1e] border-t border-white/10 px-6 py-4 flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 bg-white/5 border border-white/10 text-white font-bold rounded-xl hover:bg-white/10 transition-all">
            Cancel
          </button>
          <button onClick={handleSave} disabled={updateMutation.isPending} className="flex-1 py-3 bg-[#fbbe15] text-[#1a1a1a] font-bold rounded-xl hover:bg-[#e5ac10] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm bg-[#222] border border-white/10 rounded-2xl p-6 text-center">
        ...
        <h3 className="text-lg font-bold text-white mb-2">Delete Restaurant?</h3>
        <p className="text-sm text-zinc-400 mb-6">
          Are you sure you want to delete <span className="text-white font-semibold">&ldquo;{name}&rdquo;</span>? This action cannot be undone.
        </p>
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-3 bg-white/5 border border-white/10 text-white font-bold rounded-xl hover:bg-white/10 transition-all">
            Cancel
          </button>
          <button onClick={onConfirm} disabled={isPending} className="flex-1 py-3 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition-all disabled:opacity-60 flex items-center justify-center gap-2">
...
function RestaurantCard({
  ...
  const coverPhoto = restaurant.photos && restaurant.photos.length > 0 ? restaurant.photos[0] : RESTAURANT_PLACEHOLDER_IMG;
  const hours = restaurant.openingHours;
  const priceLabel = restaurant.priceLevel ? PRICE_LABELS[restaurant.priceLevel] || "" : "";
    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/15 transition-all group">
      ...
      <div className="relative h-40 bg-white/3">
        {coverPhoto === RESTAURANT_PLACEHOLDER_IMG ? (
          ...
          <Image src={coverPhoto} alt={restaurant.name} fill className="object-cover" />
        )}
        {/* Overlay gradient */}
        ...
        <div className="absolute top-3 right-3">
          <button onClick={() => setMenuOpen(!menuOpen)} className="w-8 h-8 rounded-lg bg-black/40 backdrop-blur-sm flex items-center justify-center hover:bg-black/60 transition-colors">
        ...
        {restaurant.avgRating > 0 && (
          <div className="absolute bottom-3 left-3 flex items-center gap-1 bg-black/40 backdrop-blur-sm px-2.5 py-1 rounded-lg">
            ...
            <span className="text-white text-xs font-bold">{restaurant.avgRating.toFixed(1)}</span>
          </div>
        )}
        ...
        {restaurant.photos && restaurant.photos.length > 1 && (
          <div className="absolute bottom-3 right-3 flex items-center gap-1 bg-black/40 backdrop-blur-sm px-2.5 py-1 rounded-lg">
            ...
            <span className="text-zinc-300 text-xs font-medium">{restaurant.photos.length}</span>
          </div>
        )}
      </div>
      ...
      <div className="p-5 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-base font-bold text-white leading-tight">{restaurant.name}</h3>
          {priceLabel && (
            <span className="text-[#fbbe15] text-sm font-bold shrink-0">{priceLabel}</span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {restaurant.cuisine && restaurant.cuisine.split(",").map((c) => (
            <span key={c.trim()} className="px-2.5 py-1 bg-[#fbbe15]/10 text-[#fbbe15] text-xs font-medium rounded-full">
              {c.trim()}
            </span>
          ))}
        </div>
        ...
        <div className="pt-2 border-t border-white/5 space-y-2">
          {restaurant.address && (
            <div className="flex items-center gap-2 text-zinc-500 text-sm">
              ...
              <span className="truncate">{restaurant.address}{restaurant.city ? `, ${restaurant.city}` : ""}</span>
            </div>
          )}
          {restaurant.phone && (
            <div className="flex items-center gap-2 text-zinc-500 text-sm">
    ...
    return (
      <MainLayout>
        <div className="w-full max-w-2xl mx-auto px-4 py-20 text-center">
          ...
          <h1 className="text-2xl font-bold text-white mb-3">Sign in to manage restaurants</h1>
          <p className="text-zinc-400 mb-8 max-w-md mx-auto">
            You need to be logged in to list and manage your restaurants on LocalBuka.
          </p>
          <button
      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-6 overflow-y-auto h-[calc(100vh-3.5rem)] md:h-auto scrollbar-hide">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-white mb-2">No restaurants yet</h2>
            <p className="text-zinc-400 text-sm mb-8 max-w-sm mx-auto">
              Get started by listing your first restaurant. It takes less than 5 minutes.
            </p>
            <Link
              href="/buka/list-resturant"
          ...
          <div className="mt-8 flex items-center gap-6 bg-white/5 border border-white/10 rounded-xl p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-zinc-300">
                <span className="font-bold text-white">{restaurants.length}</span> restaurant{restaurants.length !== 1 ? "s" : ""}
              </span>
            </div>
            <div className="flex items-center gap-2">
