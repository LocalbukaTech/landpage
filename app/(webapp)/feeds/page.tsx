...
function HomeContent() {
  ...
  const activeData = feedType === 'following' ? personalisedData : chronologicalData;
  const isLoading = feedType === 'following' ? isLoadingPersonalised : isLoadingChronological;
  const isError = feedType === 'following' ? isErrorPersonalised : isErrorChronological;
      ...
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-6 left-0 right-0 z-50 flex justify-center items-center gap-4 pointer-events-none">
          <button
            ...
            className={cn(
              ...
              feedType === 'following' ? "text-white scale-110" : "text-white/60"
            )}
          >
            Following
          </button>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-full w-full text-white/70 space-y-4">
            ...
            <p className="font-medium text-sm drop-shadow-md">Loading feed...</p>
          </div>
        ) : isError ? (
          <div className="flex flex-col items-center justify-center h-full w-full text-white/70 space-y-4">
            <p className="font-medium text-sm drop-shadow-md">Failed to load feed. Please try again.</p>
            <button 
