...
function OtherProfileContent() {
  ...
  const { data: profileResponse, isLoading: isLoadingProfile } = useUserProfile(userId || "");
  const { data: postsResponse, isLoading: isLoadingPosts } = useUserPosts(userId || "", { page: 1, pageSize: 50 });

  const userData = (profileResponse as any)?.data?.data || (profileResponse as any)?.data;
  const posts = ((postsResponse as any)?.data?.data || (postsResponse as any)?.data || []) as any[];
