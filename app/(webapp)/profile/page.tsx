...
function ProfileContent() {
  ...
  const apiUser = ((meResponse as any)?.data?.data || (meResponse as any)?.data || authUser) as any;

  const { data: userPostsResponse, isLoading: isLoadingPosts } = useUserPosts(apiUser?.id, { page: 1, pageSize: 50 });
  const { data: savedPostsResponse, isLoading: isLoadingSaved } = useSavedPosts({ page: 1, pageSize: 50 });

  const displayPosts = useMemo(() => {
    if (activeTab === "repost" || activeTab === "tagged") return [];
    if (activeTab === "saved") {
      return (savedPostsResponse?.data?.data || []).map((item: any) => item.post || item);
    }
  ...
  return (
    <MainLayout>
      <div className="w-full max-w-4xl mx-auto px-4 py-6 overflow-y-auto h-[calc(100vh-3.5rem)] md:h-auto">
        <ProfileHeader userData={apiUser} />
        <ProfileTabs 
          posts={displayPosts} 
