...
export default function RewardsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'transactions' | 'referrals' | 'how-it-works'>('transactions');
  ...
  const nairaEquivalent = wallet?.nairaBalance ?? wallet?.nairaEquivalent ?? Math.floor(currentPoints / 2);
  const minPoints = wallet?.minRedeemablePoints ?? 1000;
  const canRedeem = wallet?.canRedeem ?? (currentPoints >= minPoints);
  const pointsNeeded = wallet?.pointsNeededToRedeem ?? Math.max(0, minPoints - currentPoints);
  const lifetimeEarned = wallet?.lifetimeEarned ?? currentPoints;
  const lifetimeEarnedNaira = wallet?.lifetimeEarnedNaira ?? Math.floor(lifetimeEarned / 2);
  const lifetimeRedeemed = wallet?.lifetimeRedeemed ?? 0;
  const lifetimeRedeemedNaira = wallet?.lifetimeRedeemedNaira ?? Math.floor(lifetimeRedeemed / 2);

  const displayCode = referral?.vanityCode || referral?.referralCode || 'MYCODE';
  const shareUrl = referral?.shareLink || (typeof window !== 'undefined' ? `${window.location.origin}/?ref=${displayCode}` : `https://localbuka.com/?ref=${displayCode}`);
        ...
        <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10'>
          <div className='flex flex-col gap-1'>
            ...
            <p className='text-xs sm:text-sm text-zinc-400 m-0'>
              Invite food lovers to LocalBuka. Earn 50 Points (₦25) for every verified friend who signs up!
            </p>
          </div>
          ...
          <div className='p-6 sm:p-8 rounded-2xl bg-[#161616] border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-xl'>
            <div className='flex flex-col gap-2 text-center sm:text-left'>
              ...
              <p className='text-xs sm:text-sm text-zinc-300 max-w-lg m-0'>
                Sign in to get your personalized referral link, earn 50 points per friend, track your conversions, and cash out directly in Naira.
              </p>
            </div>
            <button
        ...
        {isAuthenticated && (
          <div className='flex flex-col gap-8'>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-12 gap-5'>
              ...
              <div className='lg:col-span-7 bg-[#161616] border border-white/10 rounded-2xl p-5 sm:p-6 flex flex-col justify-between gap-5 relative overflow-hidden shadow-lg'>
                ...
                <div className='pt-3.5 border-t border-white/10 flex flex-col gap-2 text-xs'>
                  {referral?.isEligibleForVanityCode || referral?.canRequestVanityCode || (referral?.completedReferrals ?? 0) >= (referral?.vanityCodeThreshold ?? 5) ? (
                    <div className='flex flex-col gap-2 w-full'>
                      <div className='flex items-center justify-between w-full'>
                        <span className='text-zinc-300 font-medium flex items-center gap-1.5'>
                          <Tag size={14} className='text-[#FBBE15]' />
                          {referral?.vanityCode ? (
                            <span>Active Vanity Code: <strong className='text-[#FBBE15] font-mono'>{referral.vanityCode}</strong></span>
                          ) : (
                            <span>Unlocked! You are eligible for a custom vanity code</span>
                          )}
                        </span>
                        <button
                          ...
                          {referral?.vanityCode ? 'Edit Custom Code' : 'Set Vanity Code →'}
                        </button>
                      </div>
                    ...
                    <div className='flex flex-col gap-2 w-full'>
                      <div className='flex items-center justify-between w-full text-zinc-400'>
                        <span className='flex items-center gap-1.5 text-xs'>
                          ...
                          Unlock custom vanity code: <strong className='text-white font-bold'>{referral?.completedReferrals ?? 0}/{referral?.vanityCodeThreshold ?? 5}</strong> completed referrals
                        </span>
                        <button
                          onClick={() => setIsVanityOpen(true)}
                          ...
                          style={{
                            width: `${Math.min(
                              ...
                              Math.max(
                                0,
                                Math.round(
                                  ((referral?.completedReferrals ?? 0) / (referral?.vanityCodeThreshold ?? 5)) * 100
                                )
                              )
                            )}%`,
                          }}
              ...
              <div className='lg:col-span-5 bg-[#161616] border border-white/10 rounded-2xl p-5 sm:p-6 flex flex-col justify-between gap-5 shadow-lg relative overflow-hidden'>
                <div className='flex flex-col gap-3'>
                  <div className='flex items-center justify-between'>
                    ...
                    <span className='text-[11px] px-2.5 py-0.5 rounded-full bg-white/5 text-zinc-400 border border-white/10 font-semibold'>
                      Min: {minPoints.toLocaleString()} pts (₦{(minPoints / 2).toLocaleString()})
                    </span>
                  </div>
                  ...
                  <div className='flex items-baseline gap-2'>
                    ...
                    <span className='text-sm text-zinc-400 font-semibold'>pts</span>
                  </div>

                  {/* Naira Value Conversion */}
                  <div className='flex items-center gap-2'>
                    <span className='text-lg font-bold text-[#FBBE15]'>
                      ≈ ₦{nairaEquivalent.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                    <span className='text-xs text-zinc-500'>current available cash</span>
                  </div>
                </div>
                ...
                <div className='grid grid-cols-2 gap-2.5 p-3 sm:p-3.5 rounded-xl bg-black/50 border border-white/10'>
                  ...
                  <div className='flex flex-col gap-0.5'>
                    ...
                    <span className='text-sm sm:text-base font-black text-white'>
                      {lifetimeEarned.toLocaleString()} <span className='text-[11px] font-normal text-zinc-400'>pts</span>
                    </span>
                    <span className='text-[11px] font-bold text-[#FBBE15]'>
                      ≈ ₦{lifetimeEarnedNaira.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                  ...
                  <div className='flex flex-col gap-0.5 border-l border-white/10 pl-3'>
                    ...
                    <span className='text-sm sm:text-base font-black text-emerald-400'>
                      ₦{lifetimeRedeemedNaira.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                ...
                <div>
                  <button
                    ...
                    <Coins size={16} />
                    {canRedeem ? 'Convert to Cash (₦)' : `Earn ${pointsNeeded} more pts to redeem`}
                  </button>
                  {!canRedeem && (
                    <p className='text-[10px] text-zinc-500 text-center mt-1.5 mb-0'>
                      Progress: {currentPoints} / {minPoints} pts (₦{(minPoints / 2).toLocaleString()})
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
              <div className='bg-[#161616] border border-white/10 rounded-xl p-4 flex items-center gap-4'>
                ...
                <div className='flex flex-col min-w-0'>
                  ...
                  <span className='text-xs text-zinc-400 truncate'>Completed &amp; Rewarded</span>
                </div>
              </div>
              ...
              <div className='bg-[#161616] border border-white/10 rounded-xl p-4 flex items-center gap-4'>
                ...
                <div className='flex flex-col min-w-0'>
                  ...
                  <span className='text-xs text-zinc-400 truncate'>Pending Verification</span>
                </div>
              </div>
              ...
              <div className='bg-[#161616] border border-white/10 rounded-xl p-4 flex items-center gap-4'>
                ...
                <div className='flex flex-col min-w-0'>
                  ...
                  <span className='text-xs text-zinc-400 truncate'>Total Friends Invited</span>
                </div>
              </div>
              ...
              <div className='bg-[#161616] border border-white/10 rounded-xl p-4 flex items-center gap-4'>
                ...
                <div className='flex flex-col min-w-0'>
                  <span className='text-2xl font-black text-white truncate'>
                    {(referral?.totalReferralEarnings ?? ((referral?.completedReferrals ?? 0) * 50)).toLocaleString()} <span className='text-xs font-normal text-zinc-400'>pts</span>
                  </span>
                  <span className='text-xs text-zinc-400 truncate'>
                    ≈ ₦{(referral?.totalReferralEarningsNaira ?? Math.floor((referral?.totalReferralEarnings ?? ((referral?.completedReferrals ?? 0) * 50)) / 2)).toLocaleString()} Referral Earnings
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className='flex flex-col gap-4'>
              ...
              {/* Tab Content */}
              {activeTab === 'transactions' && (
                <TransactionHistoryTable initialTransactions={recentTransactions} />
              )}
        ...
        {copiedType && (
          <div className='fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-5 py-2.5 bg-[#48bb78] text-white rounded-full shadow-2xl transition-all duration-300 animate-in fade-in slide-in-from-bottom-4'>
            <span className='text-xs font-bold tracking-wide'>
              Referral {copiedType === 'code' ? 'code' : 'link'} copied to clipboard!
            </span>
