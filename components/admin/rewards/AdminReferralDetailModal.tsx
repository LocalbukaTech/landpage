...
export function AdminReferralDetailModal({
  ...
  const { data: referral, isLoading } = useAdminReferralDetail(
    referralId || "",
  );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "COMPLETED":
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-bold bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-200 dark:border-gray-700">
          ...
          {isLoading || !referral ? (
            ...
            <>
                ...
                <div className="flex items-center gap-6">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-extrabold text-emerald-600 dark:text-emerald-400">
                      +{referral.referrerRewardPoints || 50} pts (₦
                      {(referral.referrerRewardPoints || 50) / 2})
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-extrabold text-[#b8860b] dark:text-[#fbbe15]">
                      +{referral.refereeRewardPoints || 20} pts (₦
                      {(referral.refereeRewardPoints || 20) / 2})
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-sm font-bold text-gray-900 dark:text-white">
                      {referral.referrer?.fullName ||
                        "User " + (referral.referrerId?.slice(0, 8) || "")}
                    </span>
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-sm font-bold text-gray-900 dark:text-white">
                      {referral.referee?.fullName ||
                        "User " + (referral.refereeId?.slice(0, 8) || "")}
                    </span>
                    <span className="text-xs text-gray-500 font-mono">
              ...
              <div className="flex flex-col gap-2.5 p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className="font-mono text-gray-900 dark:text-white font-semibold truncate max-w-[180px]"
                      title={referral.refereeDeviceId || ""}
                    >
              ...
              <div className="flex items-center justify-between text-xs text-gray-400 pt-2">
                ...
                {referral.updatedAt && (
                  <span>Last Updated: {formatDate(referral.updatedAt)}</span>
                )}
              </div>
            </>
          )}
