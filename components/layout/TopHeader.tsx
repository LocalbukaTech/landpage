  ...
  return (
    <div className='flex flex-col w-full border-b border-[#1F1F1F] lg:border-none'>
      ...
      <div className='lg:hidden w-full px-4 pt-4 pb-2 flex items-center gap-3'>
        ...
        <Link
          href='/dashboard'
          className='w-9 h-9 shrink-0 flex items-center justify-center rounded-full bg-transparent hover:bg-[#252525] transition-colors text-white border-2 border-white'>
      ...
      <div className='w-full pl-4 pr-4 lg:pl-14 lg:pr-12 pt-2 lg:pt-[152px] pb-4 lg:pb-10 flex items-center gap-6 lg:gap-14 overflow-x-auto scrollbar-hide'>
        {navItems.map((item) =>
          item.href ? (
            <Link
              key={item.label}
              ...
              className={cn(
                ...
                item.active
                  ? 'text-white'
                  : 'text-[#9CA3AF] hover:text-gray-200',
              )}>
              {item.label}
            </Link>
          ) : (
