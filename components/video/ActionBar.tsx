...
import { useSharePost, useRepostPost } from "@/lib/api/services/posts.hooks";
...
export function ActionBar({ post, onCommentClick, onLikeToggle, onSaveToggle, hideFollowButton }: ActionBarProps) {
  ...
  const isAlreadyFollowed = followingList.some((u: any) => u.id === post?.user?.id);
  ...
  const [savesCount, setSavesCount] = useState(post?.saveCount ?? post?.savesCount ?? 0);

  const [isReposted, setIsReposted] = useState(post?.isReposted || false);
  const [repostsCount, setRepostsCount] = useState(post?.repostCount ?? post?.repostsCount ?? 0);
  const [sharesCount, setSharesCount] = useState(post?.shareCount ?? post?.sharesCount ?? 0);
  ...
  }, [post?.id, post?.isLiked, post?.likeCount, post?.likesCount, post?.isSaved, post?.saveCount, post?.savesCount, post?.isReposted, post?.repostCount, post?.repostsCount, post?.shareCount, post?.sharesCount]);
    ...
    <div className="flex flex-col gap-4 pb-4 items-center">
      ...
      <div className="relative mb-2 mt-4 cursor-pointer hover:opacity-90 active:scale-95 transition-all">
        <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-white/80 bg-zinc-800">
          <img 
            src={post?.user?.avatar || post?.user?.profilePicture || "/images/profile.png"} 
      ...
      {actions.map((action) => (
        ...
        >
          ...
          <h6 className="text-xs font-medium text-white">{formatCount(action.count || 0)}</h6>
        </button>
      ))}
